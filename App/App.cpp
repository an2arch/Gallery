...
void App::_startTransactionScreen() {
    ...
    if (m_storage.getState().intent_next_screen) {
        // если нужно, чекаем, следует ли сохранять текущий экран
        // в стек
        ...
        // пытаемся получить прошлый экран из стека
        if (bool err = _pullStackCurrentScreen(); !err) {
            // если стек пуст, значит закрылся последний экран программы
            // придётся её закрыть
            m_is_run = false;
        }
